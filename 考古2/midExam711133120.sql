...
*/
--【Solution】
select distinct left(c.地址,3) as 居住縣市
	, 類別名稱, 產品名稱
...
*/
--【Solution】
select top 2 員工編號, 姓名, 出生日期
	,datediff(year,e.出生日期,getdate()) as 年齡
	,dense_rank() over(order by datediff(year,e.出生日期,getdate())desc) as 排名
from 員工 as e
union
select top 2 員工編號, 姓名, 出生日期
	,datediff(year,e.出生日期,getdate()) as 年齡
	,dense_rank() over(order by datediff(year,e.出生日期,getdate())asc) as 排名
from 員工 as e
go
...
from (select *
from 員工 as e
where datediff(year,e.任用日期, getdate()) > 25 ) as e
     left join (select *
from 訂單 as o
where year(o.訂貨日期) = 2006
) as o on e.員工編號=o.員工編號
where o.員工編號 is null
order by 年資 desc, e.員工編號 asc
go
...
select o.員工編號, e.姓名 as 員工姓名, o.客戶編號, c.公司名稱 as 客戶名稱, 
       o.訂單編號, 產品名稱, 建議單價, 實際單價, round(建議單價*0.9,2) as 折數
...
select distinct s.供應商編號, 供應商名稱
from (select * from 訂單 as o where year(o.訂貨日期)=2005) as o
	right join 訂單明細 as od on o.訂單編號 = od.訂單編號
	right join 產品資料 as p on od.產品編號 = p.產品編號
	right join 供應商 as s on p.供應商編號 = s.供應商編號
...
select p.類別編號, 類別名稱, 產品編號, 產品名稱, 建議單價
		, '第00'+cast(rank() over(partition by p.類別編號 order by 建議單價 desc)as varchar)+'名' as 單價排名
...
select 年齡, 血糖, 血壓, BMI, 結果, 
		(select avg(血糖) as 平均血糖
from diabetes as d
where 年齡 >= 50 and 結果=1) as 患糖尿病人的平均血糖
from (select *
from diabetes as d
where 年齡 >= 50 and 結果=0) as d
where d.血糖 >  (select avg(血糖) as 平均血糖
from diabetes as d
where 年齡 >= 50 and 結果=1)
 go
